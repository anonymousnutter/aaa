...
void potionHUD(Entity player) {
    ...
    for (int i = 0; i < effects.size(); i++) {
        ...
        render.text2d(displayName, x + TEXT_PADDING, currentY, 1, currentColor, true);
        render.text2d(durationText, x + TEXT_PADDING, currentY + fontHeight + 1, 1, WHITE, true);
...
void fpsHUD() {
    ...
    render.text2d("FPS: " + client.getFPS(), pos[0], pos[1], 1, currentColor, true);
}
...
void bpsHUD(Entity player) {
    ...
    render.text2d(String.format("%.2f blocks/sec", player.getBPS()), pos[0], pos[1], 1, currentColor, true);
}
...
void buildInfoHUD() {
    ...
    String buildInfoText = "&7" + buildVersion + " - &r" + cachedUID + " &7- " + cachedUsername;
    render.text2d(util.color(buildInfoText), pos[0], pos[1], 1, WHITE, true);
}
...
void renderSimpleWatermark(String text, boolean syncColors, int x, int y) {
    ...
    render.text2d(String.valueOf(firstLetter), currentX, y, 1f, firstColor, true);
    currentX += render.getFontWidth(String.valueOf(firstLetter));
    render.text2d(restOfText, currentX, y, 1f, WHITE, true);
}
...
void renderRoundedWatermark(String text, boolean syncColors, int x, int y) {
    ...
    if (text.contains("$SEPARATOR")) {
        ...
        for (int i = 0; i < parts.length; i++) {
            ...
            render.text2d(parts[i], currentX, textY, 1f, WHITE, true);
        ...
        render.text2d(text, textX, textY, 1f, WHITE, true);
    }
}
...
void renderTextWithSeparators(String text, float x, float y, float lineY1, float lineY2) {
    ...
    for (int i = 0; i < parts.length; i++) {
        if (i == 0 && parts[i].isEmpty()) continue;
        render.text2d(parts[i], currentX, y, 1f, WHITE, true);
        currentX += render.getFontWidth(parts[i]);
        if (i < parts.length - 1) {
            currentX += render.getFontWidth(" ") / 2;
...
void coordinatesHUD(Entity player) {
    ...
    render.text2d(String.format("%.0f, %.0f, %.0f", vec.x, vec.y, vec.z), pos[0], pos[1], 1, currentColor, true);
}
...
void renderDefaultTarget(Entity target, float thealth, float shealth, int winning, int x, int y) {
    ...
    // name
    render.text2d(target.getDisplayName(), x + ENTITY_AREA_WIDTH, y + OUTER_PADDING + 2, 1f, 0xFFFFFFFF, true);

    // health stats
    int textColor = targetHUDSync ? getCurrentColor(0) : 0xFFFFFFFF;
    render.text2d(String.format("%.1f \u2764", target.getHealth()), x + ENTITY_AREA_WIDTH,
    ...
    render.text2d(diffText, diffTextX, y + OUTER_PADDING + render.getFontHeight() + INNER_PADDING + 2,
            1f, diffColor, true);
    ...
    items[0] = target.getHeldItem();
    items[1] = target.getArmorInSlot(3); // helmet
    items[2] = target.getArmorInSlot(2); // chestplate
    items[3] = target.getArmorInSlot(1); // leggings
    items[4] = target.getArmorInSlot(0); // boots

    for (ItemStack item : items) {
        if (item != null) {
            render.item(item, currentX, itemY, 1.0f);
        }
    ...
    render.roundedRect(healthBarX, healthBarY, healthBarX + (int)(healthBarWidth * healthPercentage),
            healthBarY + OUTER_PADDING, 3f, textColor);
}
...
void renderAstolfo(Entity target, float thealth, float shealth, int winning, int x, int y) {
    ...
    render.text2d(target.getDisplayName(), nameX, nameY, 1f, 0xFFFFFFFF, true);
    ...
    if (targetShowWL) {
        ...
        render.text2d(winText, winTextX, nameY, 1f, 0xFFFFFFFF, true);
    }
    ...
    render.text2d(String.format("%.1f \u2764", target.getHealth()), nameX, midY, 2f, textColor, true);
    ...
    if (targetShowHealthDiff) {
        ...
        int diffColor = (diff > 0) ? 0xFF00FF00 : (diff < 0) ? 0xFFFF0000 : 0xFFFFFF00;
        render.text2d(diffText, diffTextX, midY, 2f, diffColor, true);
    }
}
...
void renderMyau(Entity target, float thealth, float shealth, int winning, int x, int y) {
    ...
    render.text2d(target.getDisplayName(),
    ...
    String winText = (winning == 1) ? util.color("&a&lW")
            ...
            : util.color("&c&lL");
    int winTextWidth = render.getFontWidth(winText);
    render.text2d(winText,
            x + effectiveHudWidth - winTextWidth - MYAU_HUD_PADDING,
            y + MYAU_HUD_PADDING, 1f, 0xFFFFFFFF, targetTextShadow);

    render.text2d(String.format("%.1f \u2764", target.getHealth()),
            contentStartX + MYAU_HUD_PADDING,
    ...
    int diffColor = (diff > 0) ? 0xFF00FF00 : (diff < 0) ? 0xFFFF0000 : 0xFFFFFF00;
    render.text2d(diffText, diffTextX, diffTextY, 1f, diffColor, targetTextShadow);
